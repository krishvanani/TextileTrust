...
import { Search as SearchIcon, MapPin, Star, ShieldCheck, ArrowRight, Loader } from 'lucide-react';
...
const Search = () => {
  ...
  const { user } = useAuth();
  const { setShowNavbarSearch } = useSearch();
  ...
  const fetchCompanies = async (query = '', cat = category) => {
    try {
      ...
      const { data } = await api.get(`/companies/search?query=${query}&category=${cat}`);
      setCompanies(data);
    } catch (error) {
  ...
  useEffect(() => {
    const handleScroll = () => {
        ...
        if (rect.bottom <= 80) { // 80px approxt navbar height
    ...
    <div className="min-h-screen pb-20 bg-white">
      ...
      <div className="bg-gray-50 rounded-b-2xl sm:rounded-b-3xl pt-24 sm:pt-28 pb-8 sm:pb-12 px-4 shadow-md border-b border-gray-100">
        <div className="container-custom max-w-4xl mx-auto text-center">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-slate-800 to-slate-600 mb-4 sm:mb-6 md:mb-8 tracking-tight drop-shadow-sm">Search Verified Companies</h1>
          ...
          <div className="relative max-w-3xl mx-auto flex flex-col gap-3 sm:gap-4">
            ...
            <form onSubmit={handleSearch} className="flex-grow flex flex-col sm:flex-row items-stretch sm:items-center bg-white rounded-2xl sm:rounded-full shadow-soft p-2 sm:p-1.5 border border-white/50 focus-within:ring-4 focus-within:ring-brand-500/5 focus-within:border-brand-500/20 transition-all duration-500 gap-2 sm:gap-0 hover:shadow-[0_8px_40px_rgb(0,0,0,0.06)] search-expand">
        ...
        <div className="space-y-3 sm:space-y-4">
            <>
              {loading && (
                ...
                [1, 2, 3].map((i) => (
                  <div key={i} className="p-4 sm:p-6 bg-gray-50 rounded-2xl border border-gray-100 shadow-md skeleton-pulse">
              ...
              {!loading && companies.map((company, index) => {
                ...
                else if (rating >= 1.0) ratingColor = "bg-rose-100 text-rose-700 border border-rose-200";

                return (
                <Link to={`/company/${company._id}`} key={company._id} className={`block group slide-in-left slide-in-delay-${Math.min(index + 1, 5)}`}>
                  <div className="p-4 sm:p-6 flex flex-col gap-4 sm:gap-0 sm:flex-row sm:items-center sm:justify-between card-hover-lift active:scale-[0.995] bg-gray-50 rounded-2xl shadow-md border border-gray-100 hover:border-brand-200/50 hover:bg-gray-50/90">
                    <div className="flex-1 w-full">
                      <div className="flex items-center justify-start mb-2">
                         ...
                         <div>
                            <h3 className="text-base sm:text-lg md:text-xl font-bold text-slate-800 mr-2 group-hover:text-brand-600 transition-colors truncate tracking-tight">{company.name}</h3>
                         </div>
                      ...
                      <div className="flex flex-wrap items-center text-slate-500 text-xs sm:text-sm mb-3 sm:mb-4 gap-1 sm:gap-0">
                        ...
                        <span className="font-mono bg-slate-50 px-1.5 sm:px-2 py-0.5 rounded text-slate-600 text-[10px] sm:text-xs tracking-wide border border-slate-100 truncate">GST: {company.gst}</span>
                      </div>
                      {user?.isSubscribed && (
                    ...
                    <div className="w-full sm:w-auto mt-2 sm:mt-0">
                      <div 
                        className="w-full sm:w-auto text-sm text-brand-600 group-hover:text-white group-hover:bg-brand-600 px-4 py-2.5 sm:px-3 sm:py-1.5 rounded-xl sm:rounded-lg border border-brand-100 group-hover:border-brand-600 font-medium flex items-center justify-center gap-1.5 transition-all min-h-[44px] touch-target"
                      >
                        View Profile
                ...
                <div className="text-center py-12 sm:py-16 md:py-24 bg-gray-50 rounded-2xl sm:rounded-3xl border border-gray-100 shadow-md px-4 fade-in-up">
                  ...
                  {user && !user.registeredCompanyId && (
                     <div className="bg-brand-50/50 p-4 sm:p-6 rounded-xl sm:rounded-2xl inline-block border border-brand-100/50 max-w-xs mx-auto">
                        <p className="text-brand-800 font-medium mb-3 sm:mb-4 text-sm">Is your business listed?</p>
