...
import { Star, ArrowRight, ArrowLeft, X, ThumbsUp, ThumbsDown, Calendar } from 'lucide-react';
    ...
    <section className="py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-slate-50 via-indigo-50/10 to-white border-t border-slate-200/60 relative overflow-hidden">
          ...
          >
             {reviews.map((review) => (
                <div 
                  ...
                  className="min-w-[280px] md:min-w-[350px] h-[360px] snap-center group relative bg-white border border-slate-200 rounded-3xl p-6 flex flex-col shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 hover:border-indigo-200 transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                >
                   ...
                   <div className="flex-grow flex flex-col z-10">
                       ...
                       {/* Review Body */}
                       <div className="relative mb-4 flex-grow flex flex-col justify-start">
                            ...
                            <div className="mt-auto flex items-center justify-between pt-2">
                                <span className="text-[12px] font-medium text-slate-400 flex items-center gap-1.5">
                                    ...
                                    {review.createdAt ? new Date(review.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' }) : 'Recent'}
                                </span>
                                {review.comment && review.comment.length > 100 && (
                                    <span className="text-indigo-600 text-[13px] font-semibold group-hover:text-indigo-700 transition-colors flex items-center gap-1">
       ...
       {selectedReview && (
         ...
         >
           ...
           {/* Modal Card */}
           <div 
             className="relative z-10 w-full max-w-lg bg-white rounded-3xl shadow-2xl shadow-black/20 overflow-hidden border border-slate-200/80 my-4 max-h-[90vh] flex flex-col"
             onClick={(e) => e.stopPropagation()}
             style={{ animation: 'modalSlideUp 0.35s cubic-bezier(0.16, 1, 0.3, 1)' }}
           >
             {/* Gradient Top Accent */}
             <div className="h-1.5 flex-shrink-0 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500" />
             
             {/* Close Button - increased z-index and explicit positioning */}
             <button 
               onClick={() => setSelectedReview(null)}
               className="absolute top-5 right-5 z-[50] w-9 h-9 rounded-full bg-slate-100/80 backdrop-blur-sm hover:bg-slate-200 flex items-center justify-center text-slate-600 hover:text-slate-900 transition-all hover:rotate-90 duration-300 shadow-sm border border-slate-200"
             ...
             <div className="p-7 sm:p-8 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 6px)' }}>
               {/* User Section - Added pr-12 to prevent overlap with X button */}
               <div className="flex items-center gap-4 mb-6 pr-12">
                 <div className="w-14 h-14 rounded-2xl overflow-hidden bg-white flex-shrink-0 border-2 border-white shadow-md ring-2 ring-indigo-100">
                   {selectedReview.user.photo ? (
                     ...
                     <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-50 text-indigo-600 font-bold text-lg">
               ...
               <div className="flex items-center gap-3 mb-6">
                 <div className="flex gap-1">
                   {[...Array(5)].map((_, i) => {
                     const activeColor = selectedReview.rating >= 5 ? 'bg-green-600' :
                                         selectedReview.rating >= 4 ? 'bg-lime-500' :
                                         selectedReview.rating >= 3 ? 'bg-yellow-400' :
                                         selectedReview.rating >= 2 ? 'bg-orange-500' : 'bg-red-600';
                     return (
                       <div 
                         key={i} 
                         className={`w-7 h-7 rounded-md flex items-center justify-center transition-transform ${i < selectedReview.rating ? activeColor : 'bg-slate-100'}`}
                       >
                         <Star className={`w-4 h-4 ${i < selectedReview.rating ? 'text-white fill-white' : 'text-slate-300'}`} />
                       </div>
                     );
                   })}
                 </div>
               ...
               {/* Review Text */}
               <div className="mb-6">
                 <p className="text-slate-700 text-base leading-relaxed break-words whitespace-pre-wrap">
               ...
               {selectedReview.wouldDealAgain !== undefined && selectedReview.wouldDealAgain !== null && (
                 <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl mb-6 border ${
               ...
               {selectedReview.createdAt && (
                 <div className="flex items-center gap-2 text-xs text-slate-400 mb-6">
                   ...
                   <span>
                     Reviewed on {new Date(selectedReview.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}
                   </span>
                 </div>
               )}
               ...
               {/* Company Footer */}
               <div 
                 className="flex items-center gap-3 cursor-pointer group/company p-3 -mx-1 rounded-xl transition-colors hover:bg-slate-50 mt-4"
                 onClick={() => {
                   setSelectedReview(null);
                   navigate(`/company/${selectedReview.company.id}`);
                 }}
               >
                 <div className="w-10 h-10 rounded-xl bg-white flex-shrink-0 flex items-center justify-center overflow-hidden border border-slate-200 shadow-sm">
                 ...
                 <div className="overflow-hidden flex-1">
                   <h5 className="font-bold text-slate-900 text-sm truncate group-hover/company:text-indigo-600 transition-colors pr-8">
                 ...
                 <div className="flex items-center gap-1 text-xs font-semibold text-indigo-500 opacity-0 group-hover/company:opacity-100 transition-all ml-2 whitespace-nowrap">
                   View Profile
